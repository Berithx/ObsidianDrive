...
1. Spring AOP와 Proxy 패턴의 차이점은 무엇인가?
   > Spring AOP와 Proxy 패턴 모두 객체의 메소드 호출을 가로채어 부가적인 기능을 추가하는데 사용됩니다. 하지만 주된 차이점은 Spring AOP는 로깅, 보안 등의 특정 횡단 관심사를 모듈화하여 여러 객체에 적용할 수 있는 반면에 Proxy 패턴은 특정 객체의 동작을 감싸서 확
