...
4. Spring AOP와 AspectJ의 차이점은 무엇인가?
   > Spring AOP는 런타임에 Proxy 객체를 생성하여 AOP 기능을 제공하는 프레임워크 기반의 AOP입니다. 주로 메서드 레벨에서 AOP를 적용하고 Proxy 기반이기에 성능과 유연성 측면에서 제한이 있습니다. 이에 반해 AspectJ는 컴파일 시점, 클래스 로드 시점, 런타임 시점에 `Aspect`를 적용할 수 있는 더 강력한 AOP
