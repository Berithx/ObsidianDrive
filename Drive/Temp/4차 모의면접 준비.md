...
7. 예외 처리 방식은 무엇인가?
   > 예외처리는 주로 `@ExceptionHandler`를 통한 특정 예외가 발생한 상황에 대해 특정 컨트롤러 또는 컨트롤러의 메서드 수준에서의 핸들링, `@ControllerAdvice` / `@RestControllerAdvice`를 통한 모든 컨트롤러에 전역적으로 적용하여 특정 예외에 대해 대해 중앙에서 관리하는 핸들링, `HandlerExceptionResolver`를 통한 모든 요청과 모든 예외에 대한 글로벌 예외 처리 핸들링이 있습니다. `@ExceptionHandler`가 구현되면 해당 예외가 구현 위치에서 핸들링되어 상위 레벨까지 전파되지 않습니다. 
