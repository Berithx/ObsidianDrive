...
4. 비동기 요청을 처리하는 방법은?
   > Spring에서 비동기 요청은 `@Async`, `DeferredResult`, `Callable`, `CompletableFuture` 등을 통해 처리할 수 있습니다. `@Async`는 메소드를 비동기적으로 실행하게 하고 메인 스레드는 이를 기다리지 않고 바로 응답을 반환할 수 있습니다. `DeferredResult`, `Callable`은 요청을 비동기적으로 처리하고 
