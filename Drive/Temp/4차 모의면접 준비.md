...
7. 예외 처리 방식은 무엇인가?
   > 예외처리는 주로 `@ExceptionHandler`를 통한 특정 예외가 발생한 상황에 대한 핸들링, `@ControllerAdvice` / `@RestControllerAdvice`를 통한 애플리케이션 전역적으로 발생하는 예외에 대해 중앙에서 관리하는 핸들링, `HandlerExceptionResolver`를 통한 모든 ㅇ
