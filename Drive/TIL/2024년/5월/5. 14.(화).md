...
- Client에서 HTTP 요청이 오면 DispatcherServlet 객체가 요청을 분석한다.
...
- API path, 즉 URL을 Controller에 작성하는 방법은 `@Controller` 애너테이션이 달려있는 Class를 생성한 뒤 `@GetMapping` 처럼 요청한 HTTP Method와 일ㅊ
