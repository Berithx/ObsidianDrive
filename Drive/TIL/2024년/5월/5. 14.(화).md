...
- 4. API path, 즉 URL을 Controller에 작성하는 방법은 `@Controller` Annotation이 달려있는 Class를 생성한 뒤 `@GetMapping` 처럼 요청한 HTTP Method와 일치하는 Annotation을 추가한 Method를 구현한다.
	- URL은 `@GetMapping("/api/hello")` 와 같이 해당 Annotation의 속성값으 전달해주면 된다.
